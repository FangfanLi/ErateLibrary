...
import sys, commands, socket, time, numpy, threading, select, pickle, Queue, erateSocket
...
def initialSetup(args=[]):
    ...
    if not configs.get('multipleInterface'):
        # configs.set('publicIP', getIPofInterface( configs.get('iface'), VPN=(configs.get('testID').startswith('VPN')) ))
